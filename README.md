...
| 任意の場所をクリック → **Ctrl + Alt + A** | カーソル位置から文末までを**要約**してから読み上げ |
...
| **Ctrl + Alt + A** | カーソル位置から文末までを生成AIで要約してから読み上げ |
...
### 要約読み上げ中のオーバーレイ + 元文書ハイライト

要約読み上げ中はオーバーレイが**2段構成**になります：

```
┌──────────────────────────────────────────┐
│ 📄 元の場所: セクション 3/8              │ ← セクション番号（コンパクト1行）
├──────────────────────────────────────────┤
│ 読み上げ中 5/12                          │ ← 今読み上げている要約の文節
│ 5G普及で都市部の通信が速くなってます。  │
└──────────────────────────────────────────┘
```

さらに**元文書（Chromium 系ブラウザの場合）の該当箇所を Ctrl+F でハイライト**します。
セクションが切り替わるたびに、ブラウザの検索バーに元セクションの先頭が入力されて
ページがその位置にスクロール+黄色マーカー表示されます。

これで「今聞いている要約が元のどこの段落のことか」が一目で分かります。
